...
- [x] **Dify dead code ถอดออกครบ (2026-07-12)** — ลบ `dify_patches/` (9 ไฟล์), `USE_OWN_RAG` flag (config+session+n8n → hardwire RAG), `_DIFY_TRIAGE_PATTERN` (dead), Dify docstrings/comments ที่บรรยาย runtime ผิด, DIFY env; n8n node "Ask Dify + Reply" → "Ask RAG + Reply" (ลบ else-branch Dify); เทส `call_dify` → `call_rag` ยิง `/internal/rag/answer`; regen-safe hand-edit `api-types.ts`. commit `df86157` บน DevFolk (pushed). **เก็บไว้:** คอลัมน์ DB `dify_conversation_id`/`dify_answer` (repurpose เป็น provider-agnostic id ใช้จริง), migrations, lineage comment ที่ถูกต้อง. **n8n live deployed + verified (2026-07-12):** patch SQLite ตรง (workflow_entity + workflow_history published version 4223cf05) → restart → ยิง synthetic webhook จริงเห็น bridge log `POST /internal/rag/answer 200` (RAG ไม่ใช่ Dify). node ใหม่ไม่อ่าน use_own_rag → backward-compatible กับ bridge image เก่าที่ยังรันอยู่ → **bridge rebuild ไม่เร่งด่วน** (จะได้ backend changes ที่ไม่กระทบพฤติกรรมตอน rebuild รอบถัดไป). n8n DB backup ก่อน patch = scratchpad `n8n_backup_prepatch.sqlite`
