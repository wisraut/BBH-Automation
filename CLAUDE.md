...
# 🛑🛑🛑 HARD RULE — ห้ามลบ/ถอดข้อมูลใดๆ โดยไม่ได้รับอนุญาตชัดเจน (เด็ดขาด) 🛑🛑🛑
# ห้าม DELETE / soft-delete / UPDATE ทับ / truncate **ข้อมูลคนไข้ / booking / record ใน DB จริงใดๆ**
# โดยไม่ได้รับคำสั่งอนุญาตชัดเจนจาก user ก่อน — **แม้เป็น "test data" ที่ Claude สร้างเองก็ห้ามลบเองต้องถามก่อนทุกครั้ง**
# ถ้าเผลอลบ/ทับ → ต้อง **กู้กลับ 100% ทันที** (soft-delete: `SET deleted_at=NULL, deleted_by=NULL`; ข้อมูลถูกทับ: restore จาก backup/ค่าเดิม)
# ก่อนแตะ DB จริงที่เป็น mutation ทุกครั้ง → ถามตัวเองว่า "อันนี้ลบ/ทับของเดิมไหม" ถ้าใช่และไม่ได้ขออนุญาต = หยุด ถาม user ก่อน
# เหตุ: 2026-07-24 user สั่งเข้มมากหลังกังวลว่าคนไข้ถูกลบ — treat เป็นกฎสูงสุด เทียบเท่า "BBH=hospital"
...
- [x] **Book RAG เข้า staff AI assistant (web `/ai`) + PDPA mask ชื่อคนไข้ (2026-07-20)** — staff AI ต่อ Book RAG (ตำรา 5 เล่ม): `is_book_domain` gate (โดเมน autoimmune/FM, เช็ค history 3 เทิร์น) → `search_books` → inject + `book_sources` ผ่าน SSE/`ChatResponse`; "โหมดแพทย์" อ้างตำราจริงพร้อม cite เล่ม/หน้า. + PDPA: `_all_patient_names()` seed `redact_text` known_names → mask ชื่อคนไข้ในบล็อก schedule (booking+Google Calendar summary) ก่อนส่ง OpenRouter (เดิมหลุด). code-review 6 แก้ครบ + security 0; deploy VM + verified live. **Track 2 เสร็จแล้ว (2026-07-23):** frontend render footnote `book_sources` (`<SourceFootnote>` answer-level source footer) + persist ถาวร (migration 0058 `book_sources JSON` บน `ai_messages` → รอด reload/ข้ามเครื่อง); code-review 8→แก้ 4 (de-dup title+page, drop dead score, harden decode), security 0; deploy VM + **verified ครบทุกชั้นด้วย real browser E2E (Playwright: login→ถามลูปัส→footnote render→reload persist)**. ดู CHANGELOG 2026-07-23
