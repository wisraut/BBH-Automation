...
Flow:
  ...
  Phase 5A — HTTP Webhook trigger (POST to Bridge server, background task → AI)
  Phase 5B — Direct function trigger (LINE mocked, AI+DB real) — รันคู่กับ 5A
...
def main():
    print("=" * 65)
    print(" Full Doctor Flow Test — LINE Hospital Bridge")
    ...
    # The analyze path uses our own LLM (ai_service -> rag/llm), reached through
    # the bridge server — so the bridge health check below covers it.
    ...
    # Own-LLM readiness (OpenRouter/Gemini) — the analyze path depends on it.
    # Non-fatal, but surfaces an LLM outage up front instead of as a confusing
    # late timeout deep inside Phase 5.
    if server_ok:
        try:
            r = httpx.post(
    ...
    # ══════════════════════════════════════════════════════════════════
    banner("[Phase 5B]  Direct Doctor Trigger (LINE intercepted, AI+DB real)")
    # ══════════════════════════════════════════════════════════════════
    # รันก่อน Phase 5A เพื่อป้องกัน AI call พร้อมกัน (อาจช้าลง)
    #
    print(f"  ℹ️  Report: {rpt_direct}  |  Doctor: {doctor_id}")
    print(f"  กำลังส่งไป AI... (อาจใช้เวลา 60-90 วิ)")
    ...
    # ══════════════════════════════════════════════════════════════════
    # รันหลัง Phase 5B เสร็จแล้ว → AI ไม่ถูก call พร้อมกัน
    ...
    # HTTP webhook test — poll DB (AI อาจยังทำงานอยู่)
    if http_webhook_fired and rpt_http:
        print(f"\n  [HTTP] report: {rpt_http}")
        print(f"  ⏳ รอ background task เสร็จ... (max 240s — AI call อาจกิน ~3 นาที)")
